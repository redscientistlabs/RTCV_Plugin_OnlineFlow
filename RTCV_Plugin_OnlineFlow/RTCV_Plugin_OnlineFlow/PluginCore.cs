...
namespace ONLINEFLOW
{
    [Export(typeof(IPlugin))]
    public class ONLINEFLOW : IPlugin, IDisposable
    {
        //-------[ Plugin metadata ]-------

        // >>> Make sure you rename BOTH the namespace and class (Very important)
        public string Description => "Twitch integration plugin";
        ...
        public string Name => nameof(ONLINEFLOW).Replace("_"," ");
        ...
        public bool Start(RTCSide side)
        {
            ...
            else if (side == RTCSide.Server || side == RTCSide.Both) // StandaloneRTC Process (RTCV UI) or Attached mode
            {
                ...
                S.GET<CoreForm>().pnAddon.Controls.Add(PluginForm);
